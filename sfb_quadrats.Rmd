...
fucus_p2 <- rqs3 %>%
  ...
  scale_fill_manual(values = c("darkolivegreen4", "darkolivegreen4"), drop=FALSE) + 
  #replace second color with "light grey" to highlight RTC sites
  theme_few() +
...
oysters_p2 <- rqs3 %>%
  ...
  scale_fill_manual(values = c("light grey", "light grey"), drop=FALSE) +
   #replace first color with "coral" to highlight RTC sites
  theme_few() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(
    angle = 90,
...
```{r oyster-scar-plot2, message = FALSE, Include=FALSE, fig.width = 6.5, fig.height = 4}
scat3 <- rqs3 %>%
  drop_na(oyster_n, oyster_scar_n) %>%
  filter(zone != "High") %>% 
  ggplot (aes(oyster_n, oyster_scar_n)) +
...
```
# Dataviz of sfbay transect + quadrat surveys
## Biodiversity: Richness(S) 

```{r biodiversityRTCsite, fig.cap = "Richness by site (all sources).", include =FALSE, message=FALSE, fig.width = 4, fig.height = 4}

biodiv_s <- ggplot(subset(rich2, scale == "site"), aes(x=bay_perim_order, y=richness, fill=RTC_sites)) + 
  geom_bar(stat = "identity", color="white")+
  scale_fill_manual(values = c("cadetblue4", "cadetblue4"))+ #"grey95", "thistle3"
  theme_few() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(
    angle = 90,
...
```{r biodiversityRTCquads, fig.cap = "Richness by site (quadrats).", include =FALSE, message=FALSE, fig.width = 4, fig.height = 4}


biodiv_q <- ggplot(subset(rich2, scale == "quad"), aes(x=bay_perim_order, y=richness, fill=RTC_sites)) + 
  geom_bar(stat = "identity", color="white")+
  scale_fill_manual(values = c("cadetblue2", "cadetblue2"))+ #"grey95", "thistle3"
  theme_few() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(
    angle = 90,
...
```{r biodiversityRTCtransects, fig.cap = "Richness by site (transect).", include =FALSE, message=FALSE, fig.width = 4, fig.height = 4}

biodiv_t <- ggplot(subset(rich2, scale == "tran"), aes(x=bay_perim_order, y=richness, fill=RTC_sites)) + 
  geom_bar(stat = "identity", color="white")+
  scale_fill_manual(values = c("cadetblue3", "cadetblue3"))+ #"grey95", "thistle3"
  theme_few() +
  theme(legend.position = "none") +
  theme(axis.text.x = element_text(
    angle = 90,
...
```{r rich-scale-site, include=TRUE, fig.cap = " Number of taxa observed at different scales. Site scale includes all taxa observed at the site from all sources, transect scale includes only sessile taxa contributing to primary cover, and quadrat scale includes all taxa observed in 1/4 m2 quadrats in two zones (*Fucus* and oyster zones. ", fig.width = 4, fig.height = 8}

ggarrange(biodiv_s + rremove("x.text") + rremove ("y.title") + ylim(0,35), biodiv_t + rremove("x.text") + ylim(0,35), biodiv_q + rremove ("y.title")+ ylim(0,35), ncol=1, nrow=3,heights= c(0.5, 0.5, 0.8), align = "v")
```

## Taxonomic richness by site type
```{r Tax-rich-type-site, include=FALSE, fig.width = 6, fig.height = 5, fig.cap = "Richness by site type (site)."}
#richness x sitetype
...
```{r Tax-rich-type, include=FALSE, fig.width = 6, fig.height = 5}
#richness x sitetype
...
```{r Tax-rich-type-tran, include=FALSE, fig.width = 6, fig.height = 5, fig.cap = "Richness by site type (transect)."}
#richness x sitetype
...
```{r rich-scale-site-type, include=TRUE, fig.cap = " Number of taxa observed at different sites and scales. See above for details regarding scales.", fig.width = 3, fig.height = 6}

ggarrange(richtype_s + rremove("x.text") + rremove ("y.title") + ylim(5,35), richtype_t + rremove("x.text") + ylim(5,35), richtype_q + rremove ("y.title")+ ylim(5,35), ncol=1, nrow=3,heights= c(0.55, 0.55, 0.6), align = "v")
```

## Species richness vs. intertidal width

```{r prep-for-scale, include=FALSE}
...
```{r rich-width-scale, include=TRUE, fig.width = 4, fig.height = 8, fig.cap = "Richness by intertidal width."}

SwidthAll <-
  ggplot(rich4, aes(x = width_avg_m, y = richness, fill = site_type2)) +
  geom_point(size = 4,
             shape = 21,
             alpha = 1) +
...
```
```{r rich-width-scale-lm, message = FALSE, Include=FALSE, fig.width = 6.5, fig.height = 4}
# SwidthAll.lm <-
#     ggplot (rich4, aes(x = width_avg_m, y = richness)) +
#   geom_point(size = 2,
#              shape = 21,
#              alpha = 1) +
#   geom_smooth (method = lm)+
...
```{r rich-slope-scale, include=TRUE, echo=FALSE, fig.width = 4, fig.height = 8, fig.cap = "Richness by intertidal slope."}

SslopeAll <-
  ggplot(rich4, aes(x = slope_avg, y = richness, fill = site_type2)) +
   geom_point(size = 4,
             shape = 21,
             alpha = 1) +
  facet_wrap (~ scale) +
...
```{r nada, include = FALSE}
# SslopeAll.lm <-
#     ggplot (rich4, aes(x = slope_avg, y = richness)) +
#   geom_point(size = 2,
#              shape = 21,
#              alpha = 1) +
#   geom_smooth (method = lm)+
...
```{r rich-rug-scale, include=TRUE, warning=FALSE, fig.width = 4, fig.height = 8, fig.cap = "Richness by intertidal rugosity."}

SrugAll <-
  ggplot(rich4, aes(x = rugosity_avg, y = richness, fill = site_type2)) +
   geom_point(size = 4,
             shape = 21,
             alpha = 1) +
  facet_wrap (~ scale) +
...
p2 <- ggplot(coverbio_site_class, aes(x = site, y = ftu, fill = coverclass)) +
  geom_tile()+
