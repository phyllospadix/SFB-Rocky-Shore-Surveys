...
```{r fucus x zone, include=TRUE, fig.width = 9, fig.height = 6}
...
oyster_p4 <- rqs5 %>%
  ...
  ggplot (aes(x = site_type, y = oyster_n_avg * 4, fill = site_type)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point(pch = 21,
             position = position_jitterdodge(
               jitter.height = 0.5,
...
ggarrange(fucus_p2 + rremove("x.text"), fucus_p3, ncol=1, nrow=2,heights= c(0.75, 1), align = "v")
